...
# 빠른 종목 버튼
quick_tickers = [
    ('042660', '한화오션'), ('005930', '삼성전자'), ('000660', 'SK하이닉스'),
    ('035420', 'NAVER'),   ('005380', '현대차'),   ('373220', 'LG에너지'),
    ('207940', '삼성바이오'),('068270', '셀트리온'),('035720', '카카오'),
    ('105560', 'KB금융'),
]
st.markdown("<div style='margin-top:10px;'></div>", unsafe_allow_html=True)
qcols = st.columns(5)
for i, (tc, tn) in enumerate(quick_tickers):
    with qcols[i % 5]:
        if st.button(f"{tc}\n{tn}", key=f"q_{tc}", use_container_width=True):
            st.session_state.ticker = tc
            st.session_state.query_name = tc
            get_investor_data.clear()
            get_stock_price.clear()
            get_news.clear()
            get_chart_data.clear()
