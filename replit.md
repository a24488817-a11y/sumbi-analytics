...
- **스캐너 쌍끌이 Kill Switch (2026-05-09)**: `quick_score()` — `inv_data` 취득 직후 `날짜` 내림차순 정렬 강제 (`_inv_sorted`). `_t0_inst`/`_t0_frgn` = index 0 (T-0) 직접 추출. `_is_ssankkl = _t0_inst > 0 AND _t0_frgn > 0` — T-0 당일 양매수일 때만 True. `"is_ssankkl"` 반환값을 기존 `"쌍끌이" in detail` 문자열 의존에서 직접 bool 체크로 교체. 과거 이력 쌍끌이가 스캐너 필터를 우회하는 문제 원천 차단.
- **투 트랙 전략 스캐너 V7.0 (2026-05-09)**: `quick_score()` 반환 딕트에 `target_price` 필드 추가 (`get_fundamentals()` TTL=3600s 캐시 경유). `ui_breakout_mode(scored)` 신설 — Track 1: Kill Switch(현재가≥목표가 컷아웃) + ①등락률 +2~5% + ②수급15+/쌍끌이 + ③vol_ratio≥1.5x, 총점 내림차순 Top 15. `ui_stealth_mode()` — 동일 Kill Switch 추가. 🔥 주도주/돌파 탭에서 기존 `ui_top15_tabs` 대신 `ui_breakout_mode` 호출로 교체.
...
- **증권 섹터 뉴스 노이즈 필터 (2026-05-09)**: `score_news(news, sector="일반")` — `sector="금융"` 시 `_SECURITIES_NOISE_KW`("매수 추천", "목표주가"(광범위), "투자의견", "종목 추천", "하향", "상향", "매수의견", "투자 의견", "종목추천" 9개)에 걸리는 뉴스를 noise_list로 강제 이동, 점수 0, reason="⚠️ 노이즈 (타사 리포트 발간 - 호재 아님)". "목표주가 유지"→"목표주가"로 광범위화, "하향"/"상향" 신규 추가. 타사 리포트 발간 뉴스가 해당 증권사 자체 호재로 오분류되는 문제 해소. `analyze_ticker()` + `quick_score()` 두 호출 지점 모두 `_detect_sector(name)` 전달. 기타 섹터는 무영향.
- **안전마진 Kill Switch (2026-05-09)**: `ui_score_card()` — `현재가 ≥ 목표주가(증권사)` 조건 발동 시 표시 점수만 -30 (`display_total = max(0, total-30)`). 원본 `r["total"]` 불변(랭킹 순위 영향 없음). ① `verdict` 강제 덮어쓰기 → "🔴 관망/매도 (안전마진 소멸 및 고평가 구간 진입)" (kill_switch 발동 즉시, 점수 무관). ② 분할 매수(45점) 이상 판정 강제 취소 → badge="관망/매도 (안전마진 소멸 및 고평가 구간 진입)" 덮어쓰기. 히어로 배너·게이지·badge·verdict 카드 모두 `display_total`+덮어쓴 텍스트 기준. 🚨 경보 카드 게이지 상단에 표시 (현재가·목표주가·괴리율 수치 포함).
...
- **치명 공시 경보 시스템 (2026-05-09)**: `_FATAL_DISC_KW` 13개 키워드(유상증자·전환사채·CB·BW·신주인수권부사채·교환사채·EB·블록딜·대량매각·최대주주 지분 매각·오버행·주식담보대출·반대매매) 신설. `score_news()` — 루프 ⓪번(최우선) 선탐지: `_FATAL_DISC_KW` 감지 시 bad_news + fatal_disclosures 동시 분류 후 continue (Tier1/Tier2 승격 원천 차단). 점수 확정 후 `fatal_disclosures` 비어있지 않으면 score=0 강제, score_label="0점 🚨 치명 공시 강제" 덮어쓰기. 반환 딕트에 `fatal_disclosures` 키 추가. `ui_score_card()` — 4대 미니카드 하단에 빨간 경보 블록(`fd-alert`) 조건부 출력: 경보 타이틀 + 키워드 배지 + 뉴스 제목 + "0점 강제" 안내.
- **KIS 시간외 단일가 (2026-05-09)**: `get_realtime_price()` — 평일 15:30 KST 이후, `KIS_APP_KEY`/`KIS_APP_SECRET` 설정 시 `FHKST01010400` 호출로 시간외 단일가 현재가 오버라이드. `_get_kis_token()` TTL=23h 캐시, `_get_kis_afterhours_price()` TTL=30s. 시총·이름은 항상 Naver 유지. KIS 실패/가격 0 → Naver 자동 폴백. 출처 표시: `_source="KIS시간외단일가"`.
